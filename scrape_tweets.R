...
query <- "select * from PostCovidWr order by datetime(created_at) desc limit 1"
log_eval(
  last_tweet <- DBI::dbGetQuery(con, query),
  level = "INFO"
)

log_info("Fetching tweets")
# The `since_id` argument is what allows us to get new tweets only.
weight_room_tweets <- get_timeline(twitter_handle, 
                                   n = 5000, 
                                   since_id = last_tweet)
...
latest_tweets <- weight_room_tweets %>%
                 ...
                 mutate(
                   created_at = with_tz(created_at, tz = 'America/Toronto'),
                   created_at = as.character(created_at)
                   ) %>% 
                 # Fail safe in case there is an overlap
                 anti_join(last_tweet)

log_info("Found {nrow(latest_tweets)} new tweets")

if (nrow(latest_tweets) > 0) {
  # Do a basic check to see fetched tweets are newer than tweets in DB
